$(document).on("pagebeforecreate","#apspliability_page", function (event) {
               if (strDevice!="iPad"){
               $("#BusinessSubIndustryName").attr("data-native-menu", "false");
              
               }
               if (strDevice=="iPad"){
               $("#apspfooter_ipad").attr("data-role", "footer");
...
$(document).on("pageshow","#apspliability_page", function( event,ui ) {
               CheckDeviceId(useridCust);
              /*var hbcontent=getcontentheight();
               $('.ui-content').css('height',hbcontent);*/
              /* if (devicename=='iPhone6'||devicename=='iPhone4s'||devicename=='iPhone5'){
               $('#apspliability_page').css('overflow','scroll');
               }*/
               if (strDevice!="iPad"){
               $('#apspfooter_ipad').hide();
               $('#apspfooter').show();
               changefooterlogo('apspfooterlogo',RegisteredFor);
               }
               else{
               $('#apspfooter_ipad').show();
               $('#apspfooter').hide();
	...
	$('#saveliabilityInsurance').off("click").on("click", function() {
                                                 if (strDevice!="iPad"){
                                                 $('#apspliability_page').css('overflow','hidden');
                                                 }
	    $.mobile.loading("show");
	 		...
	 		$.ajax({		
				...
				success: function (data) {	
					...
					if(issuccess==true)
					{
						...
						sessionStorage.setItem('SearchListtype',"Liability");
	 	 ...
	 	 else
	 	 {
	 		 ...
	 		 $('#InfopopupBusiness').popup('open');
               ...
               $('#OpenGallery').on("click", function () {
                                    ...
                                    getdocImages("fileInputliability","getfileconversion","InfopopupBusiness");
                                    });
               
               $('#OpenCamera').on("click", function () {
                                   $('#ConfirmPictureInfopopup').popup('close');
                                   getdocImagescamera("fileInputliability","getfileconversion","InfopopupBusiness");
                                   });
...
function CountSicCodeliability()
{
